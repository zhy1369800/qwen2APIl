...
async def collect_completion_run(
    ...
    *,
    ...
    def _finalize_result(*, reason: str | None = None, explicit_blocked_tool: str | None = None) -> RuntimeExecutionResult:
        ...
        native_snapshot = snapshot_native_function_call(native_state)
        if not detected_tool_calls and request.tools and native_snapshot is not None:
            recovered_native = recover_confirmed_native_tool_call(
                RuntimeAttemptState(native_function_call=native_snapshot),
                request,
            )
            if recovered_native:
                detected_tool_calls = [recovered_native]
                final_finish_reason = "tool_calls"
                log.info(
                    "[Collect] ✓ native function_call 快照恢复工具调用: tool=%s",
                    recovered_native.get("name"),
                )
