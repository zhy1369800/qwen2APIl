...
async def collect_completion_run(
    ...
    *,
    ...
    def _finalize_result(*, reason: str | None = None, explicit_blocked_tool: str | None = None) -> RuntimeExecutionResult:
        ...
        # 抢救逻辑：如果流结束了还没存进 native_tool_calls，但 last_function_call 还在，且文本里有拦截信息，强制捞回来
        if not native_tool_calls and last_function_call and last_function_call.get("name"):
            if "does not exist" in answer_text.lower() or explicit_blocked_tool:
                import uuid
                try:
                ...
                native_tool_calls.append(rescued)
                log.info("[Collect] ✓ 在 _finalize_result 成功抢救拦截工具: %s", rescued["name"])
